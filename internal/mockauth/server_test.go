...
func TestHealthUsesEnvelope(t *testing.T) {
	cfg := &Config{
	...
	if res.Code != http.StatusOK {
		t.Fatalf("health status: got %d", res.Code)
	}
...
func TestAuthorizeErrorIncludesMetadata(t *testing.T) {
	cfg := &Config{
		Issuer:          "http://mock.local",
		DefaultUserID:   "u1",
		DefaultUserName: "tester",
	...
	req := httptest.NewRequest(http.MethodGet, "/oauth2/authorize?response_type=code", nil)
	res := httptest.NewRecorder()
	handler.ServeHTTP(res, req)

	if res.Code != http.StatusBadRequest {
		t.Fatalf("authorize error status: got %d", res.Code)
	}
	...
	if payload["display_message"] == "" {
		t.Fatalf("expected display_message in oauth error")
	}
	if payload["trace_id"] == "" {
		t.Fatalf("expected trace_id in oauth error")
	}
}
